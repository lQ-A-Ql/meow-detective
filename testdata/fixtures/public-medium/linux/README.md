...
`testdata/fixtures/public-medium/linux/` is the home for medium-sized Linux forensic artifacts sourced from controlled VM snapshots and reviewed synthetic builders. **Committed so far:** the synthetic systemd journal fixture under `journal/` (see section 1). The remaining artifact types below are still acquisition plans; their parser regression coverage is currently carried by unit/integration tests only.
...
**Status: committed (synthetic baseline) + planned (VM snapshot)**

#### 1a. Committed synthetic fixture

**Source files:**
- `journal/system.journal` — generated by `fixture_spec()` in `crates/artifacts-linux/tests/common/mod.rs` (regenerate via `cargo test -p artifacts-linux --test journal_fixture -- --ignored`)
- `journal/expected.json` — assertion baseline per `docs/expected-json-contract.md`

**Metadata (per `docs/fixture-handbook.md`):**

- Chain: Linux artifacts / systemd journal
- Visibility: public-medium
- Source: synthetic (spec-correct builder mirroring `journal-def.h` / `compress.c`)
- Legal: fully synthetic, no real-user data; reusable in repo
- SHA-256: `8c41374b12e539d86d54c206a19cf87df21d739458e6a3fa0896139d952474c7` (`journal/system.journal`)
- Size: 36,840 bytes (110 entries)
- Coverage:
  - 110 entries across 2 distinct boot IDs (60 + 50)
  - All 8 syslog priorities, per-unit PIDs, `MESSAGE`/`PRIORITY`/`_PID`/`_UID`/`_SYSTEMD_UNIT`/`_HOSTNAME`/`SYSLOG_IDENTIFIER`/`_BOOT_ID` on every entry
  - One XZ-compressed long MESSAGE (entry 10 — the RHEL7/CentOS7 systemd v219 default) and one LZ4-compressed MESSAGE (entry 20) decoded in the same file
  - Jenkins lookup3 payload hashes (regular, non-keyed layout)
- Sensitive fields: none (all values are synthetic host/unit names)
- Expected JSON: `journal/expected.json`
- Regression test: `crates/artifacts-linux/tests/journal_fixture.rs`
- Notes:
  - Regular (non-COMPACT) layout only; COMPACT and KEYED_HASH layouts are covered by in-memory synthetic tests in `crates/artifacts-linux/tests/journal.rs`
  - Not suitable for damaged/rotated journal validation; a truncated-tail case is covered by unit tests only

#### 1b. Planned VM snapshot fixture
...
**SHA-256 (placeholder — VM snapshot fixture):**
```
