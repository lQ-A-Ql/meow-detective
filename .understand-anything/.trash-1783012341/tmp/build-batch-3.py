...
# FileIconWithStatusOverlay calls getFileIcon from file-icons.ts
edges.append({
...
# Search calls useSearchResults from features/search/hooks.ts (in-batch)
edges.append({
...
# Timeline calls useTimelineEvents from features/timeline/hooks.ts (cross-batch, neighborMap confirmed)
edges.append({
...
# Artifacts calls artifact hooks (cross-batch, neighborMap confirmed)
edges.append({
...
assert len(node_ids) == len(set(node_ids)), f'DUPLICATE node IDs: {len(node_ids)} != {len(set(node_ids))}'
...
out_path = 'D:/process/forensic/.understand-anything/intermediate/batch-3.json'
...
with open(out_path, 'r', encoding='utf-8') as f:
    verify = json.load(f)
print(f'Verified: {len(verify["nodes"])} nodes, {len(verify["edges"])} edges')
