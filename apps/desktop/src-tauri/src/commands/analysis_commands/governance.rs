use tauri::State;
use transport::dto::{
    CaseOverviewSnapshotDto, CorrelationSnapshotDto, V2GovernanceSnapshotDto,
...
#[tauri::command]
pub async fn get_case_overview_snapshot(
    state: State<'_, AppState>,
) -> Result<CaseOverviewSnapshotDto, CommandError> {
    run_active_case_command(state.inner().clone(), |connection, active| {
        app_services::v3_governance_service::get_case_overview_snapshot_for_case(
            connection,
            &active.case_root,
            &active.case_id,
        )
