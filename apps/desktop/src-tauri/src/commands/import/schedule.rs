...
pub async fn import_data_source(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let _job_id_str = match request.source_kind {
...
/// Schedule a Linux cluster import job for the active case.
pub fn schedule_linux_cluster_import_for_active_case(
    ...
    let job_id_str = job_id.0.clone();
    if let Some(app) = app {
        event_bridge::emit_job_created(app, &job_id_str, "Import Linux cluster");
    }
    job_repo
        .update_progress(
            &job_id,
