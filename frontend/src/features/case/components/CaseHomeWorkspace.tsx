  ...
  if (!model.currentCase) {
    return (
      <CaseWelcomeForms
        ...
        onRequestDeleteCase={model.requestDeleteCase}
      />
    );
  }
  ...
  return (
    <div className="flex h-full w-full flex-1 flex-col overflow-hidden bg-forensics-surface">
      <ConfirmationDialog
        ...
        description={deleteTarget?.kind === 'case'
          ? t('caseHome.confirm.deleteCaseDescription', { name: deleteTarget.value.name })
          : t('caseHome.confirm.deleteDataSourceDescription', { name: deleteTarget?.value.name ?? '' })}
      ...
      <div className="shrink-0 border-b border-forensics-border bg-forensics-panel p-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="mb-1 font-serif text-2xl tracking-tight text-forensics-text">{t('caseHome.workspace.caseNumber', { number: model.currentCase.number ?? '-' })}</div>
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-[10px] uppercase tracking-wider text-forensics-muted">
              <span className="border border-forensics-border-strong bg-forensics-surface px-2 py-1">{t('caseHome.workspace.active')}</span>
              <span className="border border-forensics-border-strong bg-forensics-surface px-2 py-1">{t('caseHome.workspace.dataSources', { count: model.metrics?.dataSourceCount ?? 0 })}</span>
              <span className="border border-forensics-warning-border bg-forensics-surface px-2 py-1">{t('caseHome.workspace.warnings', { count: model.warnings?.length ?? 0 })}</span>
              <span className="border border-forensics-warning-border bg-forensics-surface px-2 py-1">{t('caseHome.workspace.partialJobs', { count: model.partialJobCount })}</span>
            </div>
          </div>
          <div className="flex gap-8 text-right">
            <div><div className="mb-1 text-[10px] uppercase tracking-wider text-forensics-muted-light">{t('caseHome.workspace.status')}</div><div className="flex items-center justify-end gap-1.5 text-[13px] text-forensics-text"><div className="h-1.5 w-1.5 rounded-none bg-forensics-text" /> {t('caseHome.workspace.activeValue')}</div></div>
            <div><div className="mb-1 text-[10px] uppercase tracking-wider text-forensics-muted-light">{t('caseHome.workspace.createdAt')}</div><div className="font-mono text-[13px] text-forensics-text">{model.currentCase.createdAt}</div></div>
            <div><div className="mb-1 text-[10px] uppercase tracking-wider text-forensics-muted-light">{t('caseHome.workspace.examiner')}</div><div className="text-[13px] text-forensics-text">{model.currentCase.examiner ?? '-'}</div></div>
      ...
      <ImportDataSourceDialog
        ...
        listLocalDisks={model.listLocalDisks}
      />
