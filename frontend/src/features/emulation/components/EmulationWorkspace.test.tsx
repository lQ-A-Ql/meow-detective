...
function createModel(overrides: Partial<EmulationWorkspaceModel> = {}): EmulationWorkspaceModel {
  return {
    ...
    refreshing: false,
    error: undefined,
    directBootConfirmationOpen: false,
    cancelDirectBoot: vi.fn(),
    confirmDirectBoot: vi.fn().mockResolvedValue(undefined),
...
describe('EmulationWorkspace', () => {
  ...
  it('shows selected PE media as the first boot route and delegates clearing it', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '清除启动/救援 ISO' }));
