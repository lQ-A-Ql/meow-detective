      ...
      <div className="min-h-0 flex-1 overflow-auto px-5 py-4 lg:px-7">
        <div className="flex min-h-0 flex-col gap-4 xl:flex-row">
          ...
          <section className="min-w-0 flex-1">
            ...
            {section === 'topology' ? <EvidenceTopologyPanel scopes={summary.data.scopes} edges={summary.data.edges} sourceMetadata={sourceMetadata} hostFacts={infrastructure.hostFacts} networkFacts={infrastructure.networkFacts} members={summary.data.members} t={t} /> : null}
...
function EvidenceWorkloadSummary({ members, t }: { members: NonNullable<ReturnType<typeof useLinuxClusterWorkspaceModel>['summary']['data']>['members']; t: TFunction }) {
  const services = [...new Set(members.flatMap((member) => member.services))];
  const containers = [...new Set(members.flatMap((member) => member.containers))];
  if (services.length === 0 && containers.length === 0) return null;
  return <section className="border border-forensics-border bg-forensics-surface p-4"><div className="font-serif text-[15px] font-light text-forensics-text">{t('infrastructure.workspace.workloads.title')}</div><div className="mt-1 text-[11px] text-forensics-muted">{t('infrastructure.workspace.workloads.description')}</div><div className="mt-3 grid gap-3 md:grid-cols-2"><div><div className="text-[10px] uppercase tracking-wide text-forensics-muted">{t('infrastructure.workspace.workloads.services')}</div><div className="mt-2 flex flex-wrap gap-1.5">{services.map((service) => <StatusBadge key={service} label={service} variant="outline" />)}</div></div><div><div className="text-[10px] uppercase tracking-wide text-forensics-muted">{t('infrastructure.workspace.workloads.containers')}</div><div className="mt-2 flex flex-wrap gap-1.5">{containers.map((container) => <StatusBadge key={container} label={container} variant="outline" />)}</div></div></div></section>;
}
