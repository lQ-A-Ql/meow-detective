...
export function RegistryExtractionPanel({
  ...
  const info = summary ?? {
    ...
    values: [],
    generatedAt: '',
    warnings: ['注册表提取结果暂不可用。'],
  };
  const columns: DenseColumn<RegistryValue>[] = [
  ...
  return (
    <ExtractionTableSection
      ...
      stats={[
        ...
        ['当前页', info.values.length.toString()],
        ['来源 Hive', new Set(info.values.map((item) => item.hivePath)).size.toString()],
      ]}
    >
