...
describe('LinuxArtifactsPanel', () => {
  ...
  it('switches to the journal tab when clicked', () => {
    render(createElement(LinuxArtifactsPanel, {}));
    fireEvent.click(screen.getByRole('button', { name: '系统日志' }));
    expect(screen.getByText('暂无 systemd 日志')).toBeDefined();
  });

  it('switches to the sudo tab when clicked', () => {
    render(createElement(LinuxArtifactsPanel, {}));
    fireEvent.click(screen.getByRole('button', { name: 'sudo' }));
  ...
  it('renders bash commands with monospace command text', () => {
    ...
    render(createElement(LinuxArtifactsPanel, { summary }));
    fireEvent.click(screen.getByRole('button', { name: 'Shell 命令' }));
