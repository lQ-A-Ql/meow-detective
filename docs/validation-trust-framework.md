...
- 当前不保证：通用 PG/CRUSH/acting-set、EC、degraded replica、multi-PV/跨 RBD LVM、clone/snapshot/encryption、CephFS、跨节点语义关联、EXT4 deleted recovery 与全部 incompat feature 组合。当前 baseline 证明成员发现、成员独立导入、宿主文件系统读取、BlueStore/RocksDB/semantic/OMAP，以及从该私有样本显式加载的三 OSD inventory 对 `vm-100-disk-0` 执行只读重建。派生独立 source DB 已枚举直接 XFS、`centos/home`、`centos/root` 共 114,260 条文件记录并通过真实文件预览；尚未独立证明已加载 inventory 等于集群完整副本集合，该结果不扩大为任意 Ceph 集群或公开 GA 承诺。
