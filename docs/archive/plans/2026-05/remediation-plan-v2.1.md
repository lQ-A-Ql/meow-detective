...
| 5.2 Dependency gate | Completed / EVTX patched / direct deps reduced | 后端 CI 保留 `cargo audit` 并新增 `cargo deny check advisories bans licenses sources`；`deny.toml` 对短期 transitive advisories 带 reason/owner/expiry，本轮新增 `scripts/check-deny-exceptions.ps1` 并接入 backend CI，强制 advisory 例外必须包含 owner、expires 和说明文本且不得过期。`RUSTSEC-2021-0153` 已不再是例外：workspace `evtx` 指向 `crates/evtx-patched`，local fork 使用 `encoding_rs`，`Cargo.lock` 不再包含 `encoding`。desktop 直接 `dirs` 依赖已从 v5 升到 v6，lockfile 不再包含旧 `dirs-sys 0.4`、`redox_users 0.4`、`windows-sys 0.48` 和 `windows-targets 0.48` 依赖组。Search stack 已从 Tantivy 0.22.1 升到 0.26.1，lockfile 不再包含 `instant`，`lru` 从 0.12.5 升到 0.16.4，`cargo audit` warning count 从 19 降到 17，并删除不再命中的 `RUSTSEC-2024-0384` deny 例外；Tauri patch stack 已从 `tauri 2.11.1` / `tauri-utils 2.9.1` 升到 `tauri 2.11.2` / `tauri-utils 2.9.2`，但 warning count 未减少。剩余 duplicate/advisory warnings 仍主要来自 Tauri/GTK/urlpattern 传递依赖。`docs/evtx-dependency-decision.md` 和 `scripts/check-evtx-dependency-decision.ps1` 现在防止该 legacy 链路回退；前端 CI 执行 `pnpm --dir frontend audit --audit-level high` |
...
| 5.5 Fixture 策略 | Completed / real E01 manual | 新增仓库内 tiny logical directory、1024-byte tiny RAW fixture、4405-byte synthetic single-segment `testdata/fixtures/public-small/e01/tiny.E01`、synthetic `testdata/fixtures/public-small/logical/Windows/System32/config/SYSTEM` / `SOFTWARE` registry hives 和 1,118,208-byte real `testdata/fixtures/public-small/evtx/system.evtx`；`scripts/generate-tiny-fixtures.ps1` 可重建 RAW/E01/Registry tiny fixture，`crates/testing` 暴露 `tiny_logical_dir()`、`tiny_raw_image()`、`tiny_e01_image()`、`tiny_registry_system_hive()`、`tiny_registry_software_hive()`、`tiny_system_evtx()`。tiny E01 只证明 reader section/table/read/seek 行为，不代表真实文件系统镜像；Registry hives 只覆盖 Analysis 定向 parser，不代表完整 Windows registry corpus；EVTX fixture 覆盖 `evtx.boot_shutdown` parser path；真实 E01 分区/文件系统慢测仍为 `FORENSICS_E01_FIXTURE` opt-in ignored tests |
...
| Task | 状态 | 当前结果 / 剩余动作 |
|---|---|---|
| 6.1 FS enum 去重 | Partial / helper expanded | 已在 `evidence-core::filesystem` 抽出 `root_node`、路径拼接、path component normalization、常见 path/file error helper、单节点/批量 child path finalization、dot/dotdot special-entry predicate，以及按 filesystem declared logical size 截断 cluster/data-run padding 的 `truncate_data_to_declared_size()` helper。本轮新增 `invalid_fs_data`、`unsupported_fs`、`unexpected_fs_eof`、`fs_out_of_memory`、`fs_node`、`fs_node_without_timestamps`，并让 FAT/exFAT/NTFS 复用这些 constructors；exFAT VolumeLabel 异常长度补回归测试，避免坏目录项触发 slice 越界 panic。targeted tests 与 clippy 通过。剩余：更深层的枚举流程仍可继续抽象，但需避免改变 public API、排序、root semantics、NTFS parent verification 和 reader 行为 |
...
- Tiny E01 fixture targeted tests：`powershell -ExecutionPolicy Bypass -File scripts\generate-tiny-fixtures.ps1` 通过，生成 `testdata/fixtures/public-small/e01/tiny.E01` 4405 bytes；`cargo test -p image-e01` 通过，4 个默认 regression tests + 5 个 ignored real-E01 slow tests；`cargo test -p testing` 通过，4 个 fixture helper tests；`cargo clippy -p image-e01 --all-targets -- -D warnings` 通过。
...
- FS error helper checkpoint：基于 `d105e84 docs: record frontend current head gate`，新增 filesystem typed error constructors，并替换 FAT/exFAT/NTFS 中同等 kind/message 的重复 `io::Error::new`；新增 exFAT oversized VolumeLabel 回归，坏 label 长度返回 `InvalidData`，不 panic。验证：`cargo test -p evidence-core filesystem` 通过，13 个 filesystem/helper tests；`cargo test -p fs-fat -p fs-exfat -p fs-ntfs` 通过，包含 fs-exfat 31 个 tests、fs-fat 10 个 tests、fs-ntfs 22 个 tests/doc-tests；`cargo test -p app-services file_service` 通过，10 个 filtered tests；`cargo test -p app-services --test file_service_real_test` 通过，7 个 tests；`cargo clippy -p evidence-core -p fs-fat -p fs-exfat -p fs-ntfs --all-targets -- -D warnings` 通过；`cargo fmt --all -- --check` 通过。
- FS child-node constructor checkpoint：基于 `b3774d3 refactor: share filesystem error helpers`，新增 `fs_node()` 和 `fs_node_without_timestamps()`，集中构造待挂 parent path 的 pathless `FsNode`；FAT directory entry、exFAT entry set、NTFS INDX entry 均复用该 helper，未改变 child path assignment、ordering、filtering 或 reader behavior。验证：`cargo test -p evidence-core filesystem` 通过，14 个 filesystem/helper tests；`cargo test -p fs-fat -p fs-exfat -p fs-ntfs` 通过；`cargo test -p app-services file_service` 通过，10 个 filtered tests；`cargo test -p app-services --test file_service_real_test` 通过，7 个 tests；`cargo clippy -p evidence-core -p fs-fat -p fs-exfat -p fs-ntfs --all-targets -- -D warnings` 通过；`cargo fmt --all -- --check` 通过。
...
cargo audit                                        # 通过；当前 targeted dependency cleanup 后为 17 个 warning-class advisories reported
