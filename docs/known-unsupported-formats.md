...
| Linux LVM/PVE | PVE cluster 语义解析执行 | 部分支持 | 集群建模、成员串行导入、宿主 `pve/root` EXT4 文件树、BlueStore OSD label、BlueFS metadata replay、RocksDB CURRENT/IDENTITY/活动 MANIFEST control-plane replay、live-SST 物理结构库存、active-WAL/WriteBatch metadata 恢复和代表 SST entry-stream foundation 已验证；完整 live-SST digest、RocksDB latest-state、onode/blob/value、RADOS/PG/object tree、VM disk reconstruction 与跨节点关联仍不支持 |
...
- PVE 集群级语义分析、完整 live-SST digest、RocksDB latest-state、Ceph BlueStore onode/blob/value、RADOS/PG/object reconstruction、VM disk reconstruction、跨节点关联；当前承诺成员发现、成员隔离、宿主文件系统 baseline、BlueStore OSD label、BlueFS metadata replay、RocksDB control-plane、live-SST 物理结构库存、active-WAL/WriteBatch metadata 恢复与代表 SST entry-stream foundation，不承诺对象文件树
