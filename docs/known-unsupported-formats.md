...
| Linux LVM/PVE | PVE cluster 语义解析执行 | 部分支持 | 集群建模、成员串行导入、宿主 `pve/root` EXT4 文件树、BlueStore/RocksDB/semantic/OMAP、source-bound RADOS range reader 已验证。私有样本显式加载的三 OSD inventory 仅在覆盖报告完整且摘要/策略指纹匹配时重建 `vm-100-disk-0`，派生独立 source DB 含直接 XFS、`centos/home`、`centos/root` 和 114,260 条文件记录。代表文件覆盖 `1,019 B` 至 `614,794,240 B`、连续/随机/文件尾 bounded range；大文件整文件 materialize 与 request-local runtime 缺陷已修复。提交 `db49698a` 的三轮统一门禁以检材3原生 XFS 和 PVE 宿主 EXT4 为固定对照，验证 viewer/media 字节一致及 source/case invalidation 冷重建；RBD cold runtime 中位为 `3.186s`，RSS delta 为 `399-448 MiB`。覆盖报告缺失、不完整、冲突或策略指纹过期，或显式多池证据无法绑定到 RBD 数据池时仅保留元数据并失败关闭。浏览器端 media 时序与容量 LRU eviction 尚未建立私有样本门禁。通用 PG/CRUSH/EC、degraded replica、multi-PV/跨 RBD LVM、clone/snapshot/encryption、CephFS 与跨节点关联仍不支持 |
...
| 移动设备 | Android 制品与动态分区 | 部分支持 | direct Android sparse 中的明文 ext4/F2FS/EROFS 支持文件树、预览、提取和通用搜索；标准 build properties、SettingsProvider XML 和 PackageManager packages.xml 支持有来源的设备与包元数据，APK 标签和 PNG/WebP launcher 图标仅按当前页受限读取。super/liblp 逻辑分区、Android 备份、SMS/MMS、联系人、应用数据、Chrome 历史、FBE/FDE/metadata encryption 及 vendor IMEI stores 仍未实现。 |
