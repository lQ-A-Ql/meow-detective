...
- PVE 集群级 BlueStore object 语义、Ceph BlueStore onode/blob/value、RADOS/PG/object reconstruction、VM disk reconstruction、跨节点关联；当前承诺成员发现、成员隔离、宿主文件系统 baseline、BlueStore OSD label、BlueFS metadata replay、RocksDB control-plane、全部 live-SST/WAL mutation recovery 与 digest-only latest-state summary，不承诺对象文件树
