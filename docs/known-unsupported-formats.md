...
| Linux LVM/PVE | PVE cluster 语义解析执行 | 部分支持 | 集群建模、成员串行导入、宿主 `pve/root` EXT4 文件树、BlueStore/RocksDB/semantic/OMAP、source-bound RADOS range reader 已验证。私有样本显式加载的三 OSD inventory 可重建 `vm-100-disk-0`，派生独立 source DB 含直接 XFS、`centos/home`、`centos/root` 和 114,260 条文件记录。代表文件覆盖 `1,019 B` 至 `614,794,240 B`、连续/随机/文件尾 bounded range；大文件整文件 materialize 与 request-local runtime 缺陷已修复。三轮统一门禁以检材3原生 XFS 和 PVE 宿主 EXT4 为固定对照，验证 viewer/media 字节一致及 source/case invalidation 冷重建；RBD cold runtime 中位为 `2.666s`，RSS delta 为 `398-448 MiB`。浏览器端 media 时序与容量 LRU eviction 尚未建立私有样本门禁。inventory 完整性尚未独立证明，通用 PG/CRUSH/EC、degraded replica、multi-PV/跨 RBD LVM、clone/snapshot/encryption、CephFS 与跨节点关联仍不支持 |
