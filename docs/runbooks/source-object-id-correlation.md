...
每个 extractor（`crates/artifacts-windows`、`crates/artifacts-linux`、`crates/artifacts-macos` 下的 parser）在产出 `Artifact`/`TimelineEvent` 时，必须把调用方传入的 `FileEntryId` 原样写回 `source_object_id`。这个字段是后续两条链路的唯一桥梁：
...
2. **检查该记录的 `source_object_id` 是否为空**：
   ...
   ```
   如果命中的正是用户报告的记录，说明产出它的 extractor 存在漏填 `source_object_id` 的缺陷——回到对应 extractor 源码（`crates/artifacts-windows/src/...`、`crates/artifacts-linux/src/...`、`crates/artifacts-macos/src/...`）检查构造 `Artifact`/`TimelineEvent` 的代码路径，并在 `extractor_source_object_id_enforcement.rs` 里补一个回归断言。
