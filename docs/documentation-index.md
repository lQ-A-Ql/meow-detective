...
| Windows/Linux 分析调度 | `docs/analysis-extraction-scheduling.md` | Windows/Linux 共用的跨数据源串行、单源有界并行解析、顺序归并与内存治理契约 |
| Windows/Linux/集群导入调度 | `docs/import-scheduling.md` | Windows/Linux 单源与 Linux 集群统一 worker、CPU/内存 admission、取消收敛和真实 PVE 验证契约 |
...
| frontend test files | 93 | `frontend/src/**/*.test.ts(x)` |
...
| `frontend/src/**/*.test.ts(x)` | 93 | Vitest 测试文件总数 |
