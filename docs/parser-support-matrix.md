...
当前另有两个私有真实样本 baseline：检材3（通过 `FORENSICS_LINUX_E01_FIXTURE` opt-in）验证 E01/RAW -> LVM direct LV -> XFS；PVE 集群目录（通过 `FORENSICS_PVE_CLUSTER_ROOT` opt-in）验证成员发现、各节点 `disk01` -> GPT -> LVM `pve/root` -> 64-bit EXT4 -> 文件树与预览，以及三个 `disk02` 的 BlueStore label metadata inventory。PVE 集群级语义分析、BlueFS/RocksDB、RADOS/PG/object reconstruction、VM disk reconstruction 和跨节点关联继续暂缓。两类私有样本均不等同于公开 fixture，也不升级公开支持等级。
...
| PVE E01 -> LVM direct root LV -> 64-bit EXT4 / BlueStore label inventory | Linux/PVE | Beta for private host baseline / Experimental for public release | `E:\pangushi\服务器` opt-in 私有集群样本 | `FORENSICS_PVE_CLUSTER_ROOT` + `pve_cluster_host_root_filesystems_enumerate_and_preview` / `pve_cluster_representative_host_imports_tree_and_previews_by_file_id` / `check-pve-cluster-import.ps1` | 成员发现与三个宿主 `pve/root` 可枚举；代表样本完整导入保留 inode size；关键文件可按 `FileEntryId` 预览；三个 BlueStore 成员以 `ready_metadata` 保存脱敏 OSD/cluster/label replica inventory 且保持零文件 | 只证明成员级宿主 OS 文件系统、独立数据库导入和单个可识别 BlueStore LV 的 label metadata inventory。不承诺混合 filesystem + BlueStore 单源、多 BlueStore LV、BlueFS/RocksDB、RADOS object/PG/VM disk reconstruction 或跨节点关联 |
