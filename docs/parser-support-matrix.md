...
| PVE E01 -> LVM direct root LV -> 64-bit EXT4 / BlueStore semantic snapshot | Linux/PVE | Beta for private host baseline / Experimental for public release | `E:\pangushi\服务器` opt-in 私有集群样本 | `FORENSICS_PVE_CLUSTER_ROOT` + `pve_cluster_host_root_filesystems_enumerate_and_preview` / `pve_cluster_representative_host_imports_tree_and_previews_by_file_id` / `check-pve-cluster-import.ps1` | 成员发现与三个宿主 `pve/root` 可枚举；关键文件可按 `FileEntryId` 预览；三个 BlueStore 成员以 `ready_metadata` 保存 OSD/BlueFS/RocksDB latest-state 及 `S/C/O/X` super/collection/onode/shard/blob/logical+physical extent/checksum/shared-ref 规范化快照，精确 semantic digest/count oracle 闭合；普通 `file_entries` 为零 | 只证明成员级宿主 OS 文件系统、独立数据库导入、单共享设备 BlueStore LV 和 metadata semantic snapshot。不打开 RocksDB runtime，不持久化 raw SST/WAL key/value、attrs value 或 checksum bytes；不承诺混合 filesystem + BlueStore 单源、多 BlueStore LV、RADOS object content、OMAP、PG/replica、RBD/VM disk reconstruction 或跨节点关联 |
