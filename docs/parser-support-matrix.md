...
| BitLocker volume layer | Windows | Experimental | `bdetogo.raw` / `bitlocker-1.dd` opt-in oracle；恢复密码 oracle 环境门控；liuyang E01 + memory 私有 opt-in oracle | 193 个 volume lib 测试 + memory/app-service/runtime/DTO/API/report 回归 | 无凭据 inspect；密码与 48 位恢复密码解锁；匹配 Windows x64 raw memory 的 opaque AES 候选恢复；五种已验证 cipher；Hex/文本/图片/文档/媒体预览；显式 source-local catalog 导入；分区级锁定；Windows Credential Manager 已验证密钥包恢复；前端真实解锁面板；HTML/JSON 非秘密 inventory | 不提交镜像、内存或凭据，使用 `FORENSICS_BITLOCKER_*_ORACLE` / `FORENSICS_LIUYANG_MEMORY_FIXTURE` | 内存候选必须通过目标卷 NTFS boot/MFT、`$UpCase`、`$Bitmap` oracle。`0x8001`、TPM、启动密钥和 clear-key 解锁不支持；无公开内存 fixture，因此等级仍保持 Experimental。 |
