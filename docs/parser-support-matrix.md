...
| 链路 | 平台 | 当前等级 | 已验证样本 | 对齐基准 | 字段承诺 | Medium Fixture | 备注 |
|---|---|---|---|---|---|---|---|
| systemd journal | Linux | Experimental | 检材3 opt-in 私有回归（非默认 CI） | `linux_e01_integration` ignored tests / `docs/pve-cluster-parsing-design.md` Stage 0 | 时间戳、消息、PID、UID、GID、可执行文件（bestEffort） | **planned** (2026-Q3) — VM snapshot, 1000+ entries, multi-boot, LZ4 compression | 解析器已按官方格式规范重写：Jenkins/SipHash 字段哈希、KEYED_HASH/COMPACT 布局、LZ4/ZSTD 解压有合成 roundtrip 回归；XZ 显式跳过计数。真实样本 fixture 仍规划中 |
