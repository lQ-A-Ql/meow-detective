...
| PVE E01 -> LVM direct root LV -> 64-bit EXT4 / BlueStore -> RADOS -> RBD derived VM | Linux/PVE | Beta for private host/RBD baseline / Experimental for public release | `E:\pangushi\服务器` opt-in 私有集群样本 | `FORENSICS_PVE_CLUSTER_ROOT` + `check-pve-cluster-import.ps1`; retained RBD case uses `FORENSICS_PVE_RBD_CASE_ROOT` | 成员发现与三个宿主 `pve/root` 可枚举；关键文件可按 `FileEntryId` 预览；三个 BlueStore 成员以 `ready_metadata` 保存 OSD/BlueFS/RocksDB/latest-state、`S/C/O/X` semantic snapshot 与 OMAP。显式加载的三 OSD inventory 重建 `vm-100-disk-0`，派生独立 source DB 枚举直接 XFS、`centos/home`、`centos/root` 共 114,260 条文件记录，并通过真实文件 range 预览 | 只证明该私有样本当前已加载 inventory 的 RBD head、单 PV LVM 与 XFS VM 文件树；尚未独立证明 inventory 集合完整。不打开 RocksDB runtime，不持久化 raw SST/WAL key/value、attrs value 或 checksum bytes；不承诺通用 PG/CRUSH/EC、degraded replica、multi-PV/跨 RBD LVM、clone/snapshot/encryption 或 CephFS |
