...
| 微信 4.x（插件形态） | Windows | Experimental | 倩倩PC镜像（4.1.8.67）私有真实样本 opt-in 回归（`FORENSICS_WECHAT_EXTRACT_DIR`）；合成 SQLite/INI 单元与宿主回归默认 CI | `plugins-src/wechat` 单元测试 / `wechat_plugin_regression` / `wechat_realdata_validation` | 安装盘点（plugin_info.ini → installVersion/installPath/pluginVersions）、账户盘点（cloud_account.txt 云会话存在性脱敏、key_info.dat 登录密钥材料盘点脱敏、kvcomm config.ini）、db_storage 数据库盘点（wxid/category/dbName/sizeBytes/encrypted）；**明文** SQLite 库深解析（tableList/rowCounts） | **planned** — 公开合成 fixture 集 | 仅以 DLL 插件形态提供（`meow.plugin.wechat`，`plugins/windows/`）；路径自过滤 `xwechat_files/*/db_storage/` 与 Tencent/xwechat 标记。4.0.3.36+ 全部 db 为 WCDB/SQLCipher 加密，密钥仅存在于运行进程内存且主动清除，纯磁盘镜像**无法离线恢复**——加密库仅盘点并附 warning，内容提取为 Unsupported（需外部密钥材料）；明文库（旧版/其它镜像）走深解析路径 |
