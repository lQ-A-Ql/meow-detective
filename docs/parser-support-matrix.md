...
| PVE E01 -> LVM direct root LV -> 64-bit EXT4 / BlueStore + BlueFS/RocksDB control-plane replay | Linux/PVE | Beta for private host baseline / Experimental for public release | `E:\pangushi\服务器` opt-in 私有集群样本 | `FORENSICS_PVE_CLUSTER_ROOT` + `pve_cluster_host_root_filesystems_enumerate_and_preview` / `pve_cluster_representative_host_imports_tree_and_previews_by_file_id` / `check-pve-cluster-import.ps1` | 成员发现与三个宿主 `pve/root` 可枚举；关键文件可按 `FileEntryId` 预览；三个 BlueStore 成员以 `ready_metadata` 保存脱敏 OSD/label/superblock、BlueFS replay metadata、RocksDB identity/active MANIFEST、column family 和 live SST metadata，且普通 `file_entries` 为零 | 只证明成员级宿主 OS 文件系统、独立数据库导入、单个共享设备布局 BlueStore LV，以及 CURRENT 指向的活动 MANIFEST control-plane replay。不读取 SST/WAL 内容，不承诺混合 filesystem + BlueStore 单源、多 BlueStore LV、RADOS object/PG/VM disk reconstruction 或跨节点关联 |
