...
| BitLocker volume layer | Windows | Experimental | `bdetogo.raw` / `bitlocker-1.dd` opt-in oracle；恢复密码 oracle 环境门控；liuyang E01 + memory 私有 opt-in oracle | 193 个 volume lib 测试 + 23 个 memory 常规测试 + app-service/runtime/DTO/API/report 回归；liuyang targeted 与生产服务 ignored 回归 | 无凭据 inspect；密码与 48 位恢复密码解锁；匹配 Windows x64 raw memory 的结构化 VMK 恢复（PDB 门控的 fvevol keyring / device context）；五种已验证 cipher；Hex/文本/图片/文档/媒体预览；显式 source-local catalog 导入；分区级锁定；Windows Credential Manager 已验证密钥包恢复；前端真实解锁面板；内存解锁后恢复密码反推（同代 VMK 时）并以 recovered/unavailable 区分呈现；HTML/JSON 非秘密 inventory | 不提交镜像、内存或凭据，使用 `FORENSICS_BITLOCKER_*_ORACLE` / `FORENSICS_LIUYANG_MEMORY_FIXTURE` | 内存路径仅从首 1 MiB bootstrap 读取 ntoskrnl CodeView GUID，命中内嵌 1077 build 符号注册表（Windows 10 10240 → Windows 11 28000，约 96% 已索引 build）后经 Object Manager / Driver 对象链定位 FVEVol keyring / device context，注册表未覆盖的 build 走版本无关 driver-object 雕刻兜底；VMK 必须通过目标卷 FVEK datum CCM tag 与 NTFS `$UpCase`/`$Bitmap` oracle，明文预言机目前仅覆盖 NTFS 卷（exFAT/FAT 卷会按 typed failure 拒绝而非误判解锁）；恢复密码反推仅在反向 datum 与内存 VMK 同代时成立，否则显式报 unavailable。`0x8001`、TPM、启动密钥和 clear-key 解锁不支持；无公开内存 fixture，因此等级仍保持 Experimental。 |
...
| Prefetch | Windows | Beta | 无 committed fixture | expected.json / 1 个 synthetic 测试 | executable、run_count 等核心字段 | **planned** (2026-Q3) — `testdata/fixtures/public-medium/prefetch/` historical + compressed .pf variants | testdata/artifacts/windows/prefetch/ 仅含 .gitkeep。历史版本与压缩变体覆盖不足。**插件形态可用**：`plugins-src/prefetch` 提供同解析器的 DLL 形态（`meow.plugin.prefetch`），经 `scripts/build-plugins.ps1` 拷贝至 exe 旁 `plugins/windows/` 后由宿主 `plugin_loader` 加载，与内置 extractor 输出深度相等 |
...
| 微信 4.x（插件形态） | Windows | Experimental | 倩倩PC镜像（4.1.8.67）私有真实样本 opt-in 回归（`FORENSICS_WECHAT_EXTRACT_DIR` 盘点层、`FORENSICS_WECHAT_DECRYPTED_DIR` 内容层）；合成 SQLite/INI 单元与宿主回归默认 CI | `plugins-src/wechat` 单元测试 / `wechat_plugin_regression` / `wechat_realdata_validation` / `wechat_content_realdata_validation` | 安装盘点（plugin_info.ini → installVersion/installPath/pluginVersions）、账户盘点（cloud_account.txt 云会话存在性脱敏、key_info.dat 登录密钥材料盘点脱敏、kvcomm config.ini）、db_storage 数据库盘点（wxid/category/dbName/sizeBytes/encrypted）；明文/解密库的**内容解析**：联系人（WeChatContact，含 remark/nickName/delete_flag）、会话（WeChatSession，unreadCount/summary/lastTimestampUtc）、聊天记录（WeChatMessage，Msg_<md5> 会话表经 Name2Id 反查归属、real_sender_id 方向判定、WCDB_CT 标记的 zstd 正文解压、正文 500 字符截断、逐消息 UTC 时间线事件）、朋友圈（WeChatMoment，容错 XML 标签扫描取 createTime/contentDesc/hasMedia）、收藏（WeChatFavorite）；message_fts/message_resource 仅盘点 | **planned** — 公开合成 fixture 集 | 仅以 DLL 插件形态提供（`meow.plugin.wechat`，`plugins/windows/`）；路径自过滤 `xwechat_files/*/db_storage/` 与 Tencent/xwechat 标记。4.0.3.36+ 全部 db 为 WCDB/SQLCipher-4 加密，密钥仅存在于运行进程内存且主动清除，纯磁盘镜像**无法离线恢复**——需结合内存 dump 恢复密钥（插件内 `keyscan` 模块已实现 crash dump 扫描+HMAC 校验，配套 `wechat_offline scan/decrypt` 工具），经 `MEOW_WECHAT_KEYS` 开发通道（指向 `{"<dbName>": "<hex-key>"}` JSON 文件，密钥绝不进 payload/log）注入后按明文路径内容解析；无密钥时加密库仅盘点并附 warning。内容解析单库产物封顶 20,000 条，超出截断并告警 |
...
当前另有两个私有真实样本 baseline：检材3（通过 `FORENSICS_LINUX_E01_FIXTURE` opt-in）验证 E01/RAW -> LVM direct LV -> XFS；PVE 集群目录（通过 `FORENSICS_PVE_CLUSTER_ROOT` opt-in）验证成员发现、各节点 `disk01` -> GPT -> LVM `pve/root` -> 64-bit EXT4 -> 文件树与预览，以及三个 `disk02` 的 BlueStore label、BlueFS replay、RocksDB control-plane/live-SST/WAL/latest-state、`S/C/O/X` semantic snapshot、OMAP catalog、source-bound RADOS range reader、bounded RBD/head-reader 和显式三 OSD inventory 驱动的 RBD 派生 VM source。该样本的 `vm-100-disk-0` 已枚举 114,260 条文件记录，并通过直接 XFS、`centos/home`、`centos/root`、614 MiB 大文件、连续/随机 range 的真实性能门禁。统一门禁同时以检材3原生 XFS 和 PVE 宿主 EXT4 为固定 oracle 对照，并验证 viewer/media 字节一致与 source/case invalidation 冷重建。当前尚未独立证明已加载 inventory 等于集群完整副本集合；通用 PG/CRUSH/EC、degraded replica、multi-PV RBD LVM、CephFS 和跨节点关联继续暂缓。两类私有样本均不等同于公开 fixture，也不升级公开支持等级。

| 链路 | 平台 | 当前等级 | 已验证样本 | 对齐基准 | 字段承诺 | Medium Fixture | 备注 |
|---|---|---|---|---|---|---|---|
| systemd journal | Linux | Experimental | 检材3 opt-in 私有回归（非默认 CI）；`testdata/fixtures/public-medium/linux/journal/system.journal` 合成 fixture 默认 CI | `linux_e01_integration` ignored tests / `docs/pve-cluster-parsing-design.md` Stage 0 / `artifacts-linux --test journal_fixture` | 时间戳、消息、PID、UID、GID、可执行文件（bestEffort） | **completed**（合成）— 110 条、双 boot、LZ4 与 XZ 压缩字段，配 `expected.json`；VM snapshot 1000+ entries 仍 planned (2026-Q3) | 解析器已按官方格式规范重写：Jenkins/SipHash 字段哈希、KEYED_HASH/COMPACT 布局、XZ/LZ4/ZSTD 解压均有合成 roundtrip 回归；真实样本 fixture 仍规划中 |
| wtmp/utmp | Linux | Experimental | 检材3 opt-in 私有回归（`/var/log/wtmp` 可预览/提取） | `linux_e01_integration` ignored tests | 用户、终端、主机、登录/登出时间、PID（bestEffort）；`lastlog`/`faillog` 按 UID 槽位解析为登录族记录（uid、终端、主机、最后登录/失败时间、失败计数与锁定状态） | **planned** (2026-Q3) — wtmp 100+ records (3+ users), btmp 10+ failed | public-small fixture 规划中。btmp 错误登录记录需公开 expected JSON 后再升级；lastlog/faillog 复用 `LinuxWtmp` 族（`recordKind` 区分来源），暂无独立 DTO |
...
| EXT4 JBD2 journal / deleted recovery | Linux | Experimental | 合成 journal/content tests；检材3 opt-in 扫描基线 | `cargo test -p app-services deleted_recovery --lib -- --test-threads=1` | JBD2 descriptor/commit/revoke、v1/v2/v3 tag、CRC32C、direct extent depth-0 删除候选；仅返回 allocation 与 SHA-256 均验证的内容 range | **planned** (2026-Q3) — public-small EXT4 journal + deleted inode fixture | complete 候选可原子导出；partial 候选只能读取连续 verified range；不承诺间接 extent、目录恢复或所有 journal checksum/feature 组合 |
| XFS internal log / deleted recovery | Linux | Experimental | 检材3 opt-in XFS log baseline：3007 records、2872 transactions、1973 deleted candidates、0 issues | `cargo test -p app-services deleted_recovery --lib -- --test-threads=1` + `FORENSICS_LINUX_E01_FIXTURE` | checkpoint/ring-wrap/log provenance、明确 `nlink=0` 删除证据；当前删除候选默认为 metadata-only，不声称未验证文件内容 | **planned** (2026-Q3) — public-small XFS log/inode fixture | 可报告 inode/path/provenance；内容读取与完整导出仅对后端明确验证的候选开放，当前真实 XFS baseline 不提供完整文件内容恢复 |
| PVE E01 -> LVM direct root LV -> 64-bit EXT4 / BlueStore -> RADOS -> RBD derived VM | Linux/PVE | Beta for private host/RBD baseline / Experimental for public release | `E:\pangushi\服务器` opt-in 私有集群样本 | `FORENSICS_PVE_CLUSTER_ROOT` + `check-pve-cluster-import.ps1`; retained preview uses `FORENSICS_PVE_RBD_PREVIEW_CASE_ROOT` + `check-pve-rbd-preview-performance.ps1` | 成员发现与三个宿主 `pve/root` 可枚举；关键文件可按 `FileEntryId` 预览；三个 BlueStore 成员以 `ready_metadata` 保存 OSD/BlueFS/RocksDB/latest-state、`S/C/O/X` semantic snapshot 与 OMAP。显式加载的三 OSD inventory 重建 `vm-100-disk-0`，派生独立 source DB 枚举直接 XFS、`centos/home`、`centos/root` 共 114,260 条文件记录。五个代表文件覆盖 `1,019 B` 至 `614,794,240 B`、连续/随机/文件尾 bounded range；提交 `db49698a` 的三轮 RBD 中位 `4x1 MiB` p95 为 `234.152ms`，大文件随机 64 KiB p95 为 `73.804ms`。检材3原生 XFS 与 PVE 宿主 EXT4 固定 oracle 对照通过；viewer/media 字节一致，source/case invalidation 冷重建与 session 收敛通过 | 只证明该私有样本当前已加载 inventory 的 RBD head、单 PV LVM、XFS VM 文件树和代表文件 range 性能；首次三 OSD runtime 初始化中位约 `3.186s`，浏览器端 media 时序和容量 LRU eviction 尚未建立私有样本门禁。RBD/native warm 比值以 `1ms` 分母噪声下限门禁，原始亚毫秒比值仅报告。尚未独立证明 inventory 集合完整。不打开 RocksDB runtime，不持久化 raw SST/WAL key/value、attrs value 或 checksum bytes；不承诺通用 PG/CRUSH/EC、degraded replica、multi-PV/跨 RBD LVM、clone/snapshot/encryption 或 CephFS |
...
| Browser History | Windows analysis / cross-platform container parser | Experimental | Supported / GA | Chromium/Firefox parser 与生产提取链路已存在；仍需公开 medium fixture、expected.json、版本边界与加密字段说明。加密字段离线解密按浏览器分链：Chrome（flag `0x01` AES-256-GCM 直接密钥 ~127-132 / `0x02` ChaCha20-Poly1305 133-136 / `0x03` CNG `Google Chromekey1` + XOR 137+，密钥表 `KNOWN_APP_BOUND_KEYS`）、Edge 及其他 Chromium（双 DPAPI 后 Chrome key blob content 即 32 字节原始 App-Bound key，无 PostProcessData）、Firefox（独立 NSS 链路，不走 DPAPI）。v10/v11 与 v20 均已在 liuyang_pc.E01 端到端验证（Chrome 与 Edge 全覆盖）；前置条件为 SYSTEM+SECURITY+SAM hive、用户与 S-1-5-18 Protect 目录、用户侧秘密（TBAL password-SHA1/明文/域备份私钥），Chrome flag-3 另需 CNG SystemKeys 文件（elevation_service.exe 可选，用于构建绑定）。无用户侧秘密时用户 MasterKey 不可恢复，v20 降级为 Unsupported |
