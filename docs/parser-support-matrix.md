...
| BitLocker volume layer | Windows | Experimental | `bdetogo.raw` / `bitlocker-1.dd` opt-in oracle | 185 个 volume lib 测试 + app-service/runtime/DTO/API 回归 | 无凭据 inspect；密码与 48 位恢复密码解锁；五种已验证 cipher；Hex/文本/图片/文档/媒体预览；显式 source-local catalog 导入；分区级锁定 | 不提交镜像，使用 `FORENSICS_BITLOCKER_*_ORACLE` | Stage 3 完成。`0x8001`、TPM、启动密钥和 clear-key 解锁不支持；Stage 4 密钥包持久化和 Stage 5 UI 未完成，因此尚未提升为 Beta。 |
