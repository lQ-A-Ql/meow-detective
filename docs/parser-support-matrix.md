...
| BitLocker volume layer | Windows | Experimental | `bdetogo.raw` / `bitlocker-1.dd` opt-in oracle；恢复密码 oracle 环境门控 | 192 个 volume lib 测试 + app-service/runtime/DTO/API/report 回归 | 无凭据 inspect；密码与 48 位恢复密码解锁；五种已验证 cipher；Hex/文本/图片/文档/媒体预览；显式 source-local catalog 导入；分区级锁定；Windows Credential Manager 已验证密钥包恢复；前端真实解锁面板；HTML/JSON 非秘密 inventory | 不提交镜像或凭据，使用 `FORENSICS_BITLOCKER_*_ORACLE` | Stage 4-6 已完成。`0x8001`、TPM、启动密钥和 clear-key 解锁不支持；未以真实私有 E01 作为公开 fixture，因此等级仍保持 Experimental。 |
