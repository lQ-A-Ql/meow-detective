...
| 日期 | 类型 | 范围 | 状态 | 结果 | 下一边界 |
|---|---|---|---|---|---|
| 2026-07-19 | Linux/CephFS | Stage 3 bounded MDS journal replay | Implemented / positive real-sample gate pending | 按固定 Ceph upstream 版本实现 JournalPointer、legacy-v1/current-v2 header、通用 Striper、legacy/resilient frame 和 rank-local boundary/event sequence；当前 incarnation 由 `up[rank] -> GID -> mds_info_t.inc` 绑定，legacy rank/epoch map 仅有界跳过。输出独立 `framing_safe_pos`、`sequence_safe_pos` 与 `namespace_safe_pos`，并以 `source_019` 在各 source DB 内持久化不含 raw payload 的 audit projection。pointer/header/event range 均绑定 source/inventory/object provenance；缺失或重复 provenance、副本冲突、截断、预算、序号冲突和 stale rank incarnation 均 fail closed。wire 12 项、layout 3 项、service replay/discovery 14 项、source-local persistence 8 项、repository 16 项测试通过 | Stage 4 解码 inode/dirfrag/dentry 与 `EMetaBlob` mutation；在 positive CephFS fixture 和 fresh map evidence 出现前，当前 PVE 样本继续保持 `indeterminate`，不得创建 CephFS source、文件树或预览入口 |
