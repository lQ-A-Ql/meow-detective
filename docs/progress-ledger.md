...
| 日期 | 类型 | 范围 | 状态 | 结果 | 下一边界 |
|---|---|---|---|---|---|
| 2026-07-27 | Windows/BitLocker | Stage 3 服务与导入编排 | Completed | source/partition scoped inspect、密码/恢复密码解锁、显式 catalog 导入、preview lease 排空锁定、非敏感 DTO 与审计已接入；Rust 核心测试、clippy、frontend typecheck/lint/API tests 通过 | Stage 4 仅持久化已验证密钥包；Stage 5 实现不进入 store/query cache 的解锁 UI |
