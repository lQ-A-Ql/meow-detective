...
> 函数债务从 17 降至 7。例外只用于本轮无法安全拆分的 parser/repository 边界，
> 到期前必须完成后续能力族拆分。
...
| 日期 | 类型 | 范围 | 状态 | 结果 | 下一边界 |
|---|---|---|---|---|---|
| 2026-07-28 | Windows/BitLocker | Stage 7 文档、最终工程审计与质量门禁 | Completed | `cargo fmt`、workspace clippy/test、前端 typecheck/lint/test/build、Stage 0-6 边界守卫、DTO/event drift、依赖安全和文档守卫全部通过；app-services 770 项（765 passed/5 ignored）、volume-bitlocker 192 项 lib 测试，前端 87 个测试文件/591 项测试通过；报告模块拆分后的 Stage 2 守卫已同步新 case-report 边界 | BitLocker 仍保持 Experimental；继续按未提交 oracle、TPM/启动密钥/clear-key 等 unsupported 边界维护 |
| 2026-07-28 | Windows/BitLocker | Stage 5-6 前端、报告与性能闭环 | Completed | 文件浏览器按真实 BitLocker 分区显示 inspect/unlock/restore/import/lock/forget 面板；密码提交后立即清空且不进入全局状态；HTML/JSON 报告输出非秘密 inventory；32 次 persisted-key restore 回归证明不重跑密码 KDF；恢复密码 oracle 已加入环境门控 ignored 测试；前端 87 个测试文件/591 项测试，volume-bitlocker 192 项测试通过 | Stage 7 已完成；后续按支持边界维护 |
| 2026-07-28 | Windows/BitLocker | Stage 4 持久化与凭据存储 | Completed | 已验证的有界 FVEK/tweak 包通过 Windows Credential Manager 真实读写删；恢复重新校验卷身份并复探测明文文件系统；lock/forget 独立、审计和 `storedKeyAvailable` 契约已接入 | 已由 Stage 5-6 完成前端、报告和性能闭环 |
| 2026-07-27 | Windows/BitLocker | Stage 3 服务与导入编排 | Completed | source/partition scoped inspect、密码/恢复密码解锁、显式 catalog 导入、preview lease 排空锁定、非敏感 DTO 与审计已接入；Rust 核心测试、clippy、frontend typecheck/lint/API tests 通过 | Stage 4 已在 2026-07-28 完成；下一边界为不进入 store/query cache 的 Stage 5 解锁 UI |
| 2026-07-20 | Linux/CephFS | Stage 4-6 namespace/layout/source publication foundation | Implemented / synthetic positive gate passed / real positive pending | namespace graph 增加 root、parent entry/inode、canonical path、duplicate dentry/path、parent-cycle、layout/inline 和 unresolved diagnostic 校验；最终 assembly 改为 opaque 结果，materialization 只接收 input 并在服务内部执行 assembler，调用方不能声明 `complete/frozen/hash`。expected dirfrag inventory 现在拒绝额外 batch，并对未登记的子目录冻结；backtrace 必须绑定 primary directory dentry，assembly digest 绑定 remote `d_type`。source capability 自动按完整 inline、全范围 sparse proof、未证明 object coverage 和 incomplete assembly 分级为 `bounded-preview/metadata-browseable/metadata-only`。独立 `ceph_fs` source DB、全局 file ID、统一 preview、publication/recovery/delete 均接入；preview registry 的 Windows 短 uptime TTL 下溢已修复。`cephfs_source_materialization` 21 项、wire assembly 8 项、preview runtime 12 项回归通过，module/function/test-layout 守卫保持零新增债务 | 生产 cluster runner 仍只执行 presence assessment，没有 materialization 调用方；尚无真实 fresh FSMap/MDSMap、dirfrag/backtrace、journal mutation closure 和 layout byte oracle。`E:\pangushi\服务器` 继续只做 indeterminate 负向验证，不创建 CephFS source |
| 2026-07-19 | Linux/CephFS | Stage 3 bounded MDS journal replay | Implemented / positive real-sample gate pending | 按固定 Ceph upstream 版本实现 JournalPointer、legacy-v1/current-v2 header、通用 Striper、legacy/resilient frame 和 rank-local boundary/event sequence；当前 incarnation 由 `up[rank] -> GID -> mds_info_t.inc` 绑定，legacy rank/epoch map 仅有界跳过。输出独立 `framing_safe_pos`、`sequence_safe_pos` 与 `namespace_safe_pos`，并以 `source_019` 在各 source DB 内持久化不含 raw payload 的 audit projection。pointer/header/event range 均绑定 source/inventory/object provenance；缺失或重复 provenance、副本冲突、截断、预算、序号冲突和 stale rank incarnation 均 fail closed。wire 12 项、layout 3 项、service replay/discovery 14 项、source-local persistence 8 项、repository 16 项测试通过 | Stage 4-6 foundation 已落地；下一边界是从真实 monitor/source inventory 采集 fresh map、补 `EMetaBlob` mutation/dirfrag/backtrace，并建立真实 layout byte oracle |
| 2026-07-19 | Linux/CephFS | Stage 2 metadata pool inventory 与只读 object range | Implemented / positive real-sample gate pending | 新增 binary-safe object classifier、hex canonical locator、单 SQLite snapshot 的有界 pool 分页、source-local metadata projection、跨 source deterministic merge，以及绑定完整 descriptor/source manifest/object identity 的 1 MiB range reader。reader 支持完整 source 集合大于可配置副本数，缺副本、stale semantic projection、元数据冲突、字节冲突、越界和溢出均 fail closed；6 项 reader、6 项 service inventory、5 项 repository 和 3 项 wire classifier 测试通过 | 从 monitor store 自动采集 map 并建立正 CephFS fixture/oracle；未满足 presence=fresh present 前不创建 CephFS source，随后进入 Stage 3 bounded MDS journal replay |
| 2026-07-19 | Linux/CephFS | Stage 0 presence proof + Stage 1 FSMap/MDSMap/pool binding foundation | Implemented / positive real-sample gate pending | Presence 三态判定保持当前 PVE 样本 indeterminate；新增 FSMap v7-v8、MDSMap v4-v5、`mds_info_t` v4-v10 有界 decoder，typed MDS state、跨 source map 一致性、pool cluster binding、descriptor provenance 和重复 evidence 幂等。legacy rank/epoch map 按上游语义解码后丢弃，当前 rank incarnation 来自 `mds_info_t.inc`。wire 8 项与 binding 7 项测试通过 | 从 monitor store 建立带 provenance 的 map 自动采集和正样本 oracle；完成前不创建 CephFS source，Stage 2 只作为 synthetic foundation 使用 |
...
- Stage 7 后续清理事实：模块 baseline 0 行、正式临时例外 4 行、函数 baseline 7 行（历史硬债务 0 个，新代码硬上限 150 行）、test-layout baseline 0 行；`app-services` 模块与函数 baseline 均为 0，所有 baseline 只允许减少，临时例外不得无审查延期。
