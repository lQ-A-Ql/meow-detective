...
| 日期 | 类型 | 范围 | 状态 | 结果 | 下一边界 |
|---|---|---|---|---|---|
| 2026-07-28 | Windows/BitLocker | Stage 4 持久化与凭据存储 | Completed | 已验证的有界 FVEK/tweak 包通过 Windows Credential Manager 真实读写删；恢复重新校验卷身份并复探测明文文件系统；lock/forget 独立、审计和 `storedKeyAvailable` 契约已接入；volume-bitlocker 191、app-services 769、desktop 83、frontend 589 项测试及 clippy/guard 通过 | Stage 5 只补前端解锁编排，不把密码写入 store/query cache |
