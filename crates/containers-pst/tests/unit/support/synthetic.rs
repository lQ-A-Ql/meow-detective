//! Synthetic PST fixture builder shared by unit tests and fixture generators.
...
pub fn build_synthetic_pst_with_messages(message_count: usize) -> Vec<u8> {
    ...
    bbt_page[2] = 0xEC;
    bbt_page[3] = 1;
    bbt_page[8..16].copy_from_slice(&2u64.to_le_bytes());
    ...
    nbt_page[2] = 0xEC;
    nbt_page[3] = 1;
    nbt_page[8..16].copy_from_slice(&4u64.to_le_bytes());
...
fn write_simple_property_context(page: &mut [u8], props: &[(u16, &str)]) {
    ...
    for (i, (prop_id, value)) in props.iter().enumerate() {
        ...
        let tag = ((*prop_id as u32) << 16) | PTYP_STRING as u32;
