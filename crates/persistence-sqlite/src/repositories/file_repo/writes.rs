...
impl FileRepo<'_> {
    ...
    pub fn insert_batch_unchecked(&self, entries: &[FileEntry]) -> DbResult<()> {
        ...
        let mut statement = self.conn.prepare_cached(
            "INSERT OR IGNORE INTO file_entries (id, parent_id, data_source_id, path, name, entry_type, size, ext, deleted, hidden, system, created_at, modified_at, accessed_at, changed_at, hash_sha256, encrypted, read_only, archive)
