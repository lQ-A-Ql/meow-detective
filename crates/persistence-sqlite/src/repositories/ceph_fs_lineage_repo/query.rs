...
pub(super) fn find(
    ...
    let Some(lineage) = conn
        .query_row(
            "SELECT derived_data_source_id, parent_ceph_scope_id, parent_storage_object_id, cluster_identity,
...
fn map_lineage(row: &rusqlite::Row<'_>) -> rusqlite::Result<CephFsDerivedLineageRecord> {
    Ok(CephFsDerivedLineageRecord {
        ...
        cluster_identity: row.get(3)?,
        filesystem_identity: row.get(4)?,
        filesystem_id: row.get(5)?,
        filesystem_name: row.get(6)?,
        fsmap_epoch: decode_u32(row.get(7)?, 7)?,
        mdsmap_epoch: decode_u32(row.get(8)?, 8)?,
        descriptor_state: row.get(9)?,
        metadata_pool_id: row.get(10)?,
        expected_replica_count: decode_u32(row.get(11)?, 11)?,
        namespace_input_sha256: row.get(12)?,
        namespace_projection_sha256: row.get(13)?,
        namespace_assembly_sha256: row.get(14)?,
        source_capability: row.get(15)?,
        namespace_schema_version: decode_u32(row.get(16)?, 16)?,
        decoder_profile: row.get(17)?,
        journal_boundary_sha256: row.get(18)?,
        lineage_fingerprint: row.get(19)?,
    })
}
