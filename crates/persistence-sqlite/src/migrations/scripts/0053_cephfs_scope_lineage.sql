-- CephFS derived catalogs are reproducible from their read-only OSD evidence.
-- Discard legacy lineage bound to the retired generic cluster table, then
-- rebuild it with a typed Ceph topology-scope parent.
DROP TABLE IF EXISTS ceph_fs_derived_pool_sources;
DROP TABLE IF EXISTS ceph_fs_derived_map_provenance;
DROP TABLE IF EXISTS ceph_fs_derived_pools;
DROP TABLE IF EXISTS ceph_fs_derived_lineage;

CREATE TABLE ceph_fs_derived_lineage (
    ...
    parent_ceph_scope_id TEXT NOT NULL
        REFERENCES linux_topology_scopes(id) ON DELETE RESTRICT,
    ...
    UNIQUE (parent_ceph_scope_id, filesystem_identity)
);

CREATE INDEX idx_ceph_fs_derived_lineage_scope
ON ceph_fs_derived_lineage(parent_ceph_scope_id, derived_data_source_id);

CREATE TABLE ceph_fs_derived_pools (
...
CREATE TABLE ceph_fs_derived_pool_sources (
    derived_data_source_id TEXT NOT NULL,
...
CREATE INDEX idx_ceph_fs_derived_pool_sources_source
ON ceph_fs_derived_pool_sources(source_data_source_id, derived_data_source_id);

CREATE TABLE ceph_fs_derived_map_provenance (
    derived_data_source_id TEXT NOT NULL
        REFERENCES ceph_fs_derived_lineage(derived_data_source_id) ON DELETE CASCADE,
    ordinal INTEGER NOT NULL CHECK (ordinal >= 0),
    source_data_source_id TEXT NOT NULL
