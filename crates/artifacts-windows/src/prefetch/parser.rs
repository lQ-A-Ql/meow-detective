...
impl PrefetchExtractor {
    ...
    fn read_utf16le_string<R: Read>(reader: &mut R, byte_len: usize) -> Option<String> {
        ...
        let end = buf.iter().position(|&b| b == 0 && buf.get(buf.iter().position(|&x| x == b).unwrap_or(0) + 1).copied().unwrap_or(1) == 0);
        let end = end.unwrap_or(buf.len());
        let chars: Vec<u16> = buf[..end].chunks_exact(2)
...
impl ArtifactExtractor for PrefetchExtractor {
    ...
    fn display_name(&self) -> &'static str { "Windows Prefetch Parser (v30)" }
    fn family(&self) -> ArtifactFamily {
        ArtifactFamily { name: "Prefetch".into(), description: Some("Windows Prefetch files (.pf) v30".into()) }
    }
    ...
    fn run(&self, ctx: ArtifactContext, sink: &mut dyn ArtifactSink) -> Result<ExtractorReport, String> {
        ...
        if &magic != b"SCCA" && &magic != b"MAM\x04" {
            return Ok(ExtractorReport { artifacts_found: 0, timeline_events: 0, errors: vec![] });
        }
        ...
        let exe_name = Self::read_utf16le_string(&mut reader, 60)
        ...
        attrs.insert("last_run_times".into(), serde_json::Value::Array(
            times_str.iter().map(|s| s.clone().into()).collect()
        ));

        let artifact = new_artifact("Prefetch", format!("Prefetch: {}", exe_name),
            format!("{} executed {} times (fmt v{})", exe_name, run_count, format_version),
            Some(&ctx.file_id), attrs);
        sink.write_artifact(artifact);

        let mut events = 0u32;
        for rt in &run_times {
            let ev = new_timeline_event(&ctx.file_id, "PROGRAM_EXECUTION", *rt,
