...
///
/// Reads the `logins` table.  The `password_value` column is DPAPI-encrypted
/// and is surfaced only as an `[encrypted N bytes]` preview.
pub fn parse_chrome_passwords(
    ...
    for row in rows {
        match row {
            Ok(p) => results.push(p),
            Err(e) => {
                tracing::warn!("skipping password row: {}", e);
            }
        }
    }
