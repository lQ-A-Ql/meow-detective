...
use crate::{ErofsError, ErofsReader};

impl FileSystemReader for ErofsReader {
    fn root(&self) -> io::Result<FsNode> {
        Ok(root_node())
    }

    fn list_children(&self, path: &str) -> io::Result<Vec<FsNode>> {
        let inode = self.resolve_path(path).map_err(ErofsError::into_io)?;
        if !inode.is_directory() {
            return Err(path_is_not_directory(path));
        }
        let entries = self.read_directory(&inode).map_err(ErofsError::into_io)?;
        let mut nodes = Vec::with_capacity(entries.len());
        for entry in entries {
            let inode = self.read_inode(entry.nid).map_err(ErofsError::into_io)?;
