...
/// Build a minimal valid Registry hive (regf format) with a single root NK cell.
pub fn build_registry_hive(hive_name: &str, last_written: DateTime<Utc>) -> Vec<u8> {
    ...
    data.extend_from_slice(&0u32.to_le_bytes()); // offset of first HBIN
    data.extend_from_slice(&0u32.to_le_bytes()); // offset of next HBIN
    let hbin_size = 4096u32;
    data.extend_from_slice(&hbin_size.to_le_bytes());

    // NK cell at offset 0x20 within HBIN
    let cell_size = -80i32;
