//! Integration tests for Linux/macOS artifact family wiring.
//!
//! Verifies that `analysis_service` recognizes the new candidate path patterns
//! and that the Linux/macOS extractors produce Artifact/TimelineEvent outputs
//! carrying the correct `source_object_id`.

use app_services::analysis_service::{
    evidence_candidates_for_categories, extract_linux_candidate, extract_macos_candidate,
    EvidenceCandidate,
};
...
fn candidate(path: &str) -> EvidenceCandidate {
    EvidenceCandidate {
        ...
        data_source_id: "ds-linux-macos-test".to_string(),
        path: path.to_string(),
        size: 1024,
...
fn linux_bash_history_extraction_produces_events() {
    let c = candidate("/home/alice/.bash_history");
    let input = "#1700000000\nls -la /home\n";
    let outcome = extract_linux_candidate(&c, input.as_bytes());
    assert_has_outputs(&outcome, &c.file_id);
}

#[test]
fn linux_wtmp_extraction_produces_events() {
    let c = candidate("/var/log/wtmp");
    let mut buf = vec![0u8; 400];
    buf[0..4].copy_from_slice(&7i32.to_le_bytes()); // USER_PROCESS
    buf[4..8].copy_from_slice(&1234i32.to_le_bytes()); // pid
    let line = b"pts/0";
    ...
    let outcome = extract_linux_candidate(&c, &buf);
    assert_has_outputs(&outcome, &c.file_id);
}

#[test]
fn linux_apt_history_extraction_produces_events() {
    let c = candidate("/var/log/apt/history.log");
...
fn linux_dpkg_log_extraction_produces_events() {
    let c = candidate("/var/log/dpkg.log");
...
fn linux_sudo_log_extraction_produces_events() {
    let c = candidate("/var/log/auth.log");
...
#[test]
fn mac_fsevents_extraction_produces_events() {
    let c = candidate("/.fseventsd/0000000000aaaaaa");
    let mut data = Vec::new();
    data.extend_from_slice(b"1SLD");
    data.resize(32, 0);
    let ts: u64 = 1_705_276_800;
    data[16..24].copy_from_slice(&ts.to_be_bytes());
    let flags: u32 = 0x0100 | 0x20000; // Created + IsFile
    data.extend_from_slice(&flags.to_le_bytes());
    data.extend_from_slice(&[0u8; 4]);
    data.extend_from_slice(b"/Users/test/Documents/new_file.txt");
    data.push(0);

    let outcome = extract_macos_candidate(&c, &data);
    assert_has_outputs(&outcome, &c.file_id);
}

#[test]
fn mac_recent_items_extraction_produces_events() {
    let c = candidate("/Users/alice/Library/Preferences/com.apple.recentitems.plist");
...
<dict>
    ...
    let outcome = extract_macos_candidate(&c, xml.as_bytes());
    assert_has_outputs(&outcome, &c.file_id);
}

#[test]
fn mac_launch_services_extraction_produces_artifacts() {
    let c = candidate("/Users/alice/Library/Preferences/com.apple.LaunchServices.plist");
    let xml = r#"<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    ...
    assert!(
        !outcome.artifacts.is_empty(),
        "LaunchServices should produce at least one artifact"
    );
    assert_has_outputs(&outcome, &c.file_id);
}

#[test]
fn candidate_discovery_recognizes_linux_and_macos_paths() {
    ...
    let paths = [
        ...
        "/etc/crontab",
        "/var/log/auth.log",
        "/.fseventsd/0000000000aaaaaa",
        "/Users/alice/Library/Preferences/com.apple.recentitems.plist",
    ];

    for (i, path) in paths.iter().enumerate() {
        conn.execute(
            "INSERT INTO file_entries (id, data_source_id, path, size, entry_type) VALUES (?1, ?2, ?3, ?4, 'file')",
            rusqlite::params![format!("file-{i}"), "ds-test", path, 1024i64],
        )
        .expect("insert");
    }
    ...
    let linux_paths: Vec<_> = linux.iter().map(|c| c.path.as_str()).collect();
    ...
    let mac = evidence_candidates_for_categories(&conn, &["MacArtifacts"]).expect("discover macos");
    let mac_paths: Vec<_> = mac.iter().map(|c| c.path.as_str()).collect();
    assert!(mac_paths.contains(&"/.fseventsd/0000000000aaaaaa"));
    assert!(mac_paths.contains(&"/Users/alice/Library/Preferences/com.apple.recentitems.plist",));
}
