//! Fixture plugin: duplicates `meow.fixture.good`'s plugin_id. The host must
//! keep the first DLL and refuse this one.
...
pub unsafe extern "C" fn meow_plugin_info() -> MeowPluginInfo {
    MeowPluginInfo {
        ...
        plugin_version: b"0.2.0\0".as_ptr(),
        display_name: b"Fixture Good Duplicate\0".as_ptr(),
        evidence_platform: MeowEvidencePlatform::Windows,
        families_json: b"[\"Fixture\"]\0".as_ptr(),
        path_patterns_json: b"[\"*.dup\"]\0".as_ptr(),
    }
}
...
pub unsafe extern "C" fn meow_plugin_extract(
    ...
    let mut buffer = br#"{"artifacts": []}"#.to_vec();
