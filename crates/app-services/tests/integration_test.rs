...
fn full_case_lifecycle() {
    ...
    std::fs::write(evidence_dir.join("readme.txt"), b"forensics analysis pipeline test").unwrap();
    ...
    active.with_conn(|conn| {
        ...
        assert!(stats.file_count >= 2, "Expected at least 2 files, got {}", stats.file_count);
        ...
        let path_map: std::collections::HashMap<String, String> = all_files.iter()
            .map(|f| (f.id.0.clone(), f.path.clone()))
            .collect();
        let ev = evidence_dir.clone();
        let to_index: Vec<domain::FileEntryId> = all_files.iter().take(10).map(|f| f.id.clone()).collect();
        let idx_result = search_service::index_files(conn, &index_dir, &to_index, move |file_id| {
            let rel_path = path_map.get(&file_id.0)?;
            let abs_path = ev.join(if rel_path.is_empty() { "." } else { rel_path });
            std::fs::File::open(&abs_path).ok().map(|r| Box::new(r) as Box<dyn std::io::Read>)
        })
            .map_err(|e| persistence_sqlite::DbError::System(e))?;
