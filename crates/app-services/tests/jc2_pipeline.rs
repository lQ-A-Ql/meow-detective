...
fn read_mft_params(path: &Path, vol_offset: u64) -> (u64, u64, u32, u16, u64) {
    ...
    let rs = match boot[0x40] as i8 {
        v if v > 0 => 1024, v if v < 0 => (1u32 << v.unsigned_abs()).max(512), _ => 1024,
    };
...
fn jc2_full_pipeline() {
    ...
    println!("检材2 probe: {} candidates, {} partitions", candidates.len(), probe.partitions.len());
    ...
    let active = case_service::create_case(
        &tmp.path().join("cases"), "jc2-pipeline", Some("tester")).unwrap();
    let case_id = active.meta.id.clone();

    active.with_conn(|conn| {
        let ds_id = domain::DataSourceId(uuid::Uuid::new_v4().to_string());
        DataSourceRepo::new(conn).insert(&case_id, &domain::DataSource {
            id: ds_id.clone(), name: "jc2-system".into(),
            kind: domain::DataSourceKind::E01, source_path: path.to_path_buf(),
        ...
        let stats = file_service::enumerate_filesystem_mft(
            conn, &ds_id, path, MAIN_NTFS_OFFSET,
            mc, cs, rs, bps, mft_size,
            Some(&|pct, msg| eprintln!("[MFT {pct}%] {msg}")), None,
        )?;
        let import_ms = t0.elapsed().as_millis();
        println!("[BENCH] import_mft: {import_ms}ms, files={}, dirs={}", stats.file_count, stats.dir_count);

        // Phase 2: Extract artifacts
        let t1 = Instant::now();
        let registry = artifact_service::create_registry();
        let boxed: Box<dyn evidence_core::EvidenceReader> = Box::new(E01Reader::open(path).unwrap());
        ...
        for (name, hive_path) in &hives {
            let mut buf = Vec::new();
            if fs.open_file(hive_path).and_then(|mut f| f.read_to_end(&mut buf)).is_ok() {
                ...
                if artifact_service::run_extractors_on_file(
                    &registry, &domain::FileEntryId(format!("jc2-{name}")), hive_path, reader, &mut sink,
                ).is_ok() && !sink.artifacts.is_empty() {
                    artifact_service::store_artifacts(conn, &sink.artifacts, &case_id.0, &ds_id.0).unwrap();
        ...
        for evtx_path in &[
            ...
            let mut buf = Vec::new();
            if fs.open_file(evtx_path).and_then(|mut f| f.read_to_end(&mut buf)).is_ok() {
                let mut sink = artifacts_core::VecSink::new();
                let reader: Box<dyn std::io::Read> = Box::new(std::io::Cursor::new(buf));
                if artifact_service::run_extractors_on_file(
                    &registry, &domain::FileEntryId("jc2-events".into()), evtx_path, reader, &mut sink,
                ).is_ok() && !sink.artifacts.is_empty() {
                    artifact_service::store_artifacts(conn, &sink.artifacts, &case_id.0, &ds_id.0).unwrap();
                    total_artifacts += sink.artifacts.len() as u32;
                    println!("  extracted EVTX {}: {} artifacts", evtx_path, sink.artifacts.len());
                }
            }
        }

        let artifact_ms = t1.elapsed().as_millis();
        println!("[BENCH] artifact_extraction: {artifact_ms}ms, total artifacts={}", total_artifacts);
        ...
        println!("[BENCH] timeline: {tl_ms}ms, items(first_page)={}", tl.items.len());

        // Phase 4: Correlation
        let t3 = Instant::now();
        let snapshot = correlation::get_correlation_snapshot(conn).unwrap();
        let corr_ms = t3.elapsed().as_millis();
        println!("[BENCH] correlation: {corr_ms}ms, nodes={} edges={} clusters={} leads={}",
            snapshot.node_count, snapshot.edge_count, snapshot.cluster_count, snapshot.lead_count);
        let covered: Vec<_> = snapshot.family_coverage.iter()
        ...
        println!("[BENCH] governance: {gov_ms}ms, score={}/100 grade={} gates={}/{}",
            gov.release_scorecard.total_score, gov.release_scorecard.grade,
            gov.release_gates.iter().filter(|g| g.status == transport::dto::ReleaseGateStatusDto::Passed).count(),
