...
pub fn run_import_analysis_staging(
    ...
    for worker_id in &worker_ids {
        ...
        if staging::get_worker_meta(&conn, "status")
        ...
        staging::set_worker_meta(&conn, "status", "pending")
            .map_err(|e| format!("Init analysis staging {worker_id}: {e}"))?;
        staging::set_worker_meta(&conn, "merged", "false")
            .map_err(|e| format!("Init analysis staging merge flag {worker_id}: {e}"))?;
    }
            ...
            Ok((_worker_id, worker_result)) => {
                ...
                if let Some(cb) = progress_cb {
                    ...
                    cb(
                        pct.min(82),
                        &format!(
                            "Analysis workers done: phase=analysis workersDone={}/{} processed={} queued={} indexed={} active={} rssMb={}",
                            completed,
                            worker_count,
    ...
    let merge_stats = staging::merge_analysis_staging_to_main(
        ...
        Some(&|completed, total| {
            if let Some(cb) = progress_cb {
                ...
                cb(
                    pct.min(94),
                    &format!("Merged analysis staging {}/{}", completed, total),
                );
            }
        }),
    )?;
