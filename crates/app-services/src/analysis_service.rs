...
/// Registry and EVTX parsers are not wired yet. Report that explicitly instead
/// of manufacturing host facts from file presence.
pub fn extract_system_info() -> AnalysisSystemInfoDto {
    AnalysisSystemInfoDto {
        computer_name: None,
        os_version: None,
        build_number: None,
        ...
        warnings: vec![
            "系统信息解析器尚未接入 Registry/EVTX；当前不会输出未验证主机事实。".to_string(),
        ],
    }
}
...
pub fn classify_files_by_magic(
    ...
    for entry in files.iter().take(sample_size as usize) {
        ...
        if let Some((file_type, category, description)) = file_type {
            let entry = classified_file(entry, size, file_type, description);
            ...
            unclassified
                .files
                .push(classified_file(entry, size, "Unknown", "Unknown file type"));
...
mod tests {
    ...
    fn system_info_is_not_fabricated_without_parsers() {
        ...
        assert!(info
            .warnings
            .iter()
            .any(|warning| warning.contains("不会输出未验证")));
    }
