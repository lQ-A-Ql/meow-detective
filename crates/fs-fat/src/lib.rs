use evidence_core::filesystem::{
    ...
    invalid_fs_data, is_special_directory_name, path_components, path_is_directory, root_node,
...
impl FatReader {
    ...
    fn bounded_range_len(file_size: u64, offset: u64, length: usize) -> io::Result<usize> {
        ...
        let bounded = requested.min(file_size.saturating_sub(offset));
    ...
    fn read_cluster_chain_range(
        ...
        let bounded_len = Self::bounded_range_len(file_size, offset, length)?;
        if bounded_len == 0 {
            return Ok(Vec::new());
        }
        ...
        let first_cluster_index = offset / self.cluster_size;
        let mut cluster_offset = offset % self.cluster_size;
        let mut cluster = start_cluster;
        let mut visited = std::collections::HashSet::new();
        ...
        while remaining > 0 {
            ...
            let available_in_cluster = self.cluster_size.saturating_sub(cluster_offset);
