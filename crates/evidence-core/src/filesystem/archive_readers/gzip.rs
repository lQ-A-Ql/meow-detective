...
pub(crate) fn skip_exact(reader: &mut dyn Read, mut amount: u64) -> io::Result<()> {
...
pub(crate) struct GzipTarEntryReader {
    source_path: PathBuf,
    tar_offset: u64,
    position: u64,
    length: u64,
...
impl GzipTarEntryReader {
    pub(crate) fn new(path: &Path, tar_offset: u64, length: u64) -> io::Result<Self> {
        let mut reader = Self {
...
impl std::io::Seek for GzipTarEntryReader {
...
pub(crate) struct GzipSingleEntryReader {
    source_path: PathBuf,
    position: u64,
    length: u64,
    decoder: MultiGzDecoder<File>,
...
impl GzipSingleEntryReader {
    pub(crate) fn new(path: &Path, length: u64) -> io::Result<Self> {
        Ok(Self {
            source_path: path.to_path_buf(),
            position: 0,
            length,
...
impl std::io::Seek for GzipSingleEntryReader {
    fn seek(&mut self, position: SeekFrom) -> io::Result<u64> {
        let next = bounded_seek(position, self.position, self.length)?;
        self.reset_and_skip(next)?;
        Ok(next)
