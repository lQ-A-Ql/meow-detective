...
fn p() -> std::path::PathBuf { "E:/pangushi/刘洋/liuyang_pc.E01".into() }
fn skip() -> bool { if !p().exists() { eprintln!("SKIP"); true } else { false } }

#[test]
fn open_real_file() { if skip() { return; } let r = E01Reader::open(&p()).unwrap(); assert!(r.info().size > 0); }
