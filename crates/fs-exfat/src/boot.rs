...
impl ExfatBootSector {
    ...
    pub fn parse(data: &[u8]) -> io::Result<Self> {
        ...
        Ok(Self {
            ...
            file_system_name: data[3..11].try_into().unwrap(),
            partition_offset: u64::from_le_bytes(data[64..72].try_into().unwrap()),
            volume_length: u64::from_le_bytes(data[72..80].try_into().unwrap()),
            fat_offset: u32::from_le_bytes(data[80..84].try_into().unwrap()),
            fat_length: u32::from_le_bytes(data[84..88].try_into().unwrap()),
            cluster_heap_offset: u32::from_le_bytes(data[88..92].try_into().unwrap()),
            cluster_count: u32::from_le_bytes(data[92..96].try_into().unwrap()),
            first_cluster_of_root: u32::from_le_bytes(data[96..100].try_into().unwrap()),
            volume_serial_number: u32::from_le_bytes(data[100..104].try_into().unwrap()),
