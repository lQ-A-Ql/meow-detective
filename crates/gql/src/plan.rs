...
mod tests {
    ...
    fn plan_for_typed_match() {
        let q = crate::parser::parse("MATCH (n:File)-[e:References]->(m:Artifact) RETURN n, e, m")
            .unwrap();
        let plan = estimate_plan(&q);
        // Should have at least: source scan, edge traverse, target filter, project
        assert!(
            plan.steps.len() >= 4,
            "Expected at least 4 steps, got {}",
            plan.steps.len()
        );
        assert!(plan.steps[0].uses_index, "Source scan should use index");
    ...
    fn plan_for_untyped_match_has_higher_cost() {
        let q_typed = crate::parser::parse("MATCH (n:File)-[e:References]->(m) RETURN n").unwrap();
        let q_untyped = crate::parser::parse("MATCH (n)-[e]->(m) RETURN n").unwrap();
        let plan_typed = estimate_plan(&q_typed);
        let plan_untyped = estimate_plan(&q_untyped);
        // Untyped scan should cost more
        assert!(
            plan_untyped.total_cost.nodes_visited > plan_typed.total_cost.nodes_visited,
            "Untyped scan nodes_visited ({}) should exceed typed scan nodes_visited ({})",
            plan_untyped.total_cost.nodes_visited,
            plan_typed.total_cost.nodes_visited
        );
    }

    #[test]
    fn plan_with_where_adds_filter_step() {
        let q_no_where = crate::parser::parse("MATCH (n:File)-[e]->(m) RETURN n").unwrap();
        let q_with_where =
            crate::parser::parse("MATCH (n:File)-[e]->(m) WHERE e.confidence > 0.7 RETURN n")
                .unwrap();
        let plan_no = estimate_plan(&q_no_where);
        let plan_with = estimate_plan(&q_with_where);
        assert!(plan_with.steps.len() > plan_no.steps.len());
    }

    #[test]
    fn plan_with_count_aggregate_has_project_step() {
        let q = crate::parser::parse("MATCH (n)-[e]->(m) RETURN count(*)").unwrap();
        let plan = estimate_plan(&q);
        let project_step = plan
            .steps
            .iter()
            .find(|s| s.operation == "project")
            .expect("Should have a project step");
        assert!(project_step.description.contains("count(*)"));
    }

    #[test]
    fn plan_with_limit() {
        let q = crate::parser::parse("MATCH (n)-[e]->(m) RETURN n LIMIT 10").unwrap();
        let plan = estimate_plan(&q);
        let limit_step = plan
            .steps
            .iter()
            .find(|s| s.operation == "limit")
            .expect("Should have a limit step");
        assert!(limit_step.description.contains("10"));
    }
    ...
    fn plan_total_cost_aggregates_all_steps() {
        ...
        let sum_ops: u64 = plan
            .steps
            .iter()
            .map(|s| s.estimated_cost.estimated_ops)
            .sum();
    ...
    fn plan_reverse_direction_uses_target_index() {
        let q = crate::parser::parse("MATCH (a:Artifact)<-[e:References]-(f:File) RETURN a, e, f")
            .unwrap();
        let plan = estimate_plan(&q);
        let traverse_step = plan
            .steps
            .iter()
            .find(|s| s.operation == "traverse")
            .expect("Should have traverse step");
        assert_eq!(
            traverse_step.index_name,
            Some("idx_graph_edges_target".to_string())
        );
    }
    ...
    fn plan_outgoing_direction_uses_source_index() {
        ...
        assert_eq!(
            traverse_step.index_name,
            Some("idx_graph_edges_source".to_string())
        );
    }
}
